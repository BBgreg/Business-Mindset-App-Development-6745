...
import { gregHeadKnowledgeBase, gregHeadPersonality } from '../data/knowledgeBase'
...
export class AIService {
  ...
  async generateInsight(userQuery, userId, conversationHistory = []) {
    try {
      ...
      const aiResponse = await this.supabaseService.generateAIResponse(
        chatHistory,
        systemPrompt,
        userId
      )
  ...
  buildContext(userQuery) {
    ...
    gregHeadKnowledgeBase.commonMistakes.forEach(mistake => {
      if (query.includes(mistake.title.toLowerCase()) ||
          query.includes(mistake.description.toLowerCase())) {
        relevantContext.mistakes.push(mistake)
      }
    })
...
What specific challenge is keeping you up at night?`
  ...
  async loadChatHistory(userId) {
    try {
      const messages = await this.supabaseService.getChatHistory(userId)
  ...
  async clearChatHistory(userId) {
    try {
      await this.supabaseService.deleteChatHistory(userId)
    } catch (error) {
